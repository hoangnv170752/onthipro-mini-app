...
function LoginPage() {
  ...
  const [email, setEmail] = useState("hoangvn.grinda@gmail.com");
  ...
  // Check if user is already logged in - wrapped in try/catch for safety
  useEffect(() => {
    try {
      if (isAuthenticated()) {
        navigate("/");
  ...
  const handleLogin = async () => {
    try {
      ...
      // Save remember me preference - wrapped in try/catch
      try {
        if (rememberMe) {
        ...
        <Box className="space-y-5">
          ...
          <Box>
            <Box className="flex justify-between mb-2">
              ...
              <Text className="text-blue-600 text-sm cursor-pointer">Quên mật khẩu?</Text>
            </Box>
